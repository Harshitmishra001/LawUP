...
- **T4 Hardware limitation:** PEFT locks LoRA adapters into `bfloat16`, causing `torch.amp.GradScaler` to crash when scaling gradients. We monkeypatched `GradScaler` to a no-op dummy scaler. This stabilized `grad_norm` at the cost of dynamic loss scaling.

### Known Technical Trade-Offs & Open Questions
1.  **Hardware Incompatibility (GradScaler vs T4 bfloat16):** The T4 GPU architecture lacks hardware support for `bfloat16`. `bitsandbytes` enforces `bfloat16` for QLoRA, causing `torch.amp.GradScaler` to crash when scaling gradients. We monkeypatched `GradScaler` to a no-op dummy scaler. This successfully stabilized `grad_norm` at the cost of dynamic loss scaling.
2.  **Untested True Checkpoint Recovery:** We proved the script *detects* checkpoints and skips steps (dry-run). We did *not* prove it can cleanly restore optimizer states and learning rate schedules mid-epoch after an actual crash, as the main run finished uninterrupted.
3.  **The `final/` vs `checkpoint-300` Discrepancy:** The `final/` adapter weights saved at the end of training were 120MB (FP32), while the checkpoints (including the objectively best `checkpoint-300`) were 60MB (bfloat16). While we confirmed `load_best_model_at_end=True` fired, it is unclear if the `final/` save explicitly upcasted `checkpoint-300` to FP32, or if it accidentally saved the raw step-382 weights before the callback fully restored the older weights. To eliminate risk, we bypassed `final/` entirely for M3 and explicitly merged `checkpoint-300/`.
